...
However it means that you can't take full advantage of QUIC via HTTP/3 otherwise your application will choke older HTTP versions.
...
And sometimes it's not worth fighting against HTTP semantics.
If you want to make an application that relies on QUIC's behavior, then you should use QUIC directly.
...
<figure>
	...
	<figcaption>Each IP packet contains the source IP/port and destination IP/port</figcaption>
</figure>
...
It's obnoxious and why you need some form of keep-alive to both detect and prevent any NAT rebinding.

**In QUIC land**, the Connection ID saves the day.
It will take 1 RTT for QUIC to verify the new path but it just works™
...
<figure>
	...
	<figcaption>chad 2-tuple doesn't care about the source address</figcaption>
</figure>
...
<figure>
	...
	<figcaption>beta 2-tuple failed to connect, oops</figcaption>
</figure>
...
Unfortunately, there's no way to identify the connection based on the DTLS header otherwise you could use it like a connection ID.
...
<figure>
	...
	<figcaption>Chad Anycast</figcaption>
</figure>
...
<figure>
	...
	<figcaption>Beta Anycast</figcaption>
</figure>
...
3.  **Datagrams can't be sent to arbitrary destinations**. It means you can't send RTP and RTCP to different ports; they have to be muxed.
...
You should stick with UDP if you need maximum control, preferably using [DTLS](https://www.rfc-editor.org/rfc/rfc9147.html) for encryption.
