...
Wow, TCP sure has a low bitrate at 30% packet loss.
...
The solution is to send *less data* to instead drain network queues. 🤯
...
WebRTC gets a passing grade because the Google engineers are super smart, although its complexity is a testament to the difficulty of networking.
...
That's why it does make sense to perform additional FEC at a higher level, but again, it should be hop-by-hop.
