...
I've flown out to 8 in-person meetings, attended at least 20 remote meetings, left hundreds of comments on issues/PRs, and even authored the original transport draft.
...
I'm going to focus on implementing cool shit instead.
...
So instead, the transport has become overly generic... and molded around Cisco's proprietary stack.
...
The idea is that receiver can detect gaps caused by loss.
In my case, I want the player and CDN to know that there's no gap between frame 5 and frame 6, avoiding the need ask upstream (ex. "does frame 5.5 exist?").
But no, the Cisco folks insist that we MUST support both explicit and implicit gaps and have yet to elaborate why.
...
The best compromise is one that nobody is happy with, but there's no need to compromise when you stack the working group.

The result so far can only be described as _obtuse_.
...
-   🎏 **Karp**: A thin layer on top of 🍴**MoqTransfork** capable of mutliple quality/latency targets. It should be implemented by generic media clients, such as VLC and OBS.
